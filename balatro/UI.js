...
function renderVoucher(canvas, voucherName) {
	const voucherData = vouchers.find(
...
`;
	...
	searchLabel.textContent =
		"Press enter to search (comma separated values, min length 4 char)";
	...
	function extractShopQueues(text) {
		...
		if (matches) {
			matches.forEach((match) => {
				...
				const queueMatch = match.match(
					/Shop Queue:([\s\S]*?)(?=Packs:|$)/
				);
	...
	function generateCombinations() {
		conditions = swaginput.value.split(", ");
		...
		while (current.length <= maxLen) {
			...
			for (let i = current.length - 1; i >= 0; i--) {
				if (current[i] !== chars[chars.length - 1]) {
					current[i] = chars[chars.indexOf(current[i]) + 1];
					break; // Break after incrementing the character
				} else {
					current[i] = chars[0]; // Reset to the first character
				}
			}

			// If all characters are at max value, add a new character to increase length
			if (current.every((char) => char === chars[0])) {
				current.unshift(chars[0]);
			}
		}
	}
	...
	function coolmatch(seedinputvalue) {
		...
		if (
			conditions.every((condition) => {
				// Check if the condition is present in the entries array
				return entries.some(
					(entry) =>
						entry.trim().toLowerCase() === condition.toLowerCase()
				);
			})
		) {
